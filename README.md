    ...
    <script type="module">
        ...
        const initAuth = async () => {
            try {
                ...
                console.error("인증 에러: Firebase Console에서 익명 로그인이 활성화되었는지 확인하세요.", err); 
            }
        };
        ...
        const createShareCode = async () => {
            ...
            try {
                ...
                currentRoomUnsub = onSnapshot(roomRef, (snapshot) => {
                    ...
                    console.error("실시간 감시 에러:", err);
                });
            } catch (err) {
                console.error("방 생성 오류:", err);
                alert("코드 생성에 실패했습니다. Firebase 보안 규칙(Rules)이나 익명 로그인 설정을 다시 확인해 보세요.");
            }
        };
        ...
        function startApp(code) {
            ...
            onSnapshot(diaryCol, (snapshot) => {
                ...
                if (diaries.length === 0) {
                    list.innerHTML = '<div class="col-span-full py-20 text-center text-slate-300 border-2 border-dashed border-slate-100 rounded-3xl">아직 기록된 이야기가 없어요. 첫 일기를 써보세요!</div>';
                    return;
                }
            ...
            }, (err) => console.error("Firestore 일기 데이터 로드 오류:", err));
        }
